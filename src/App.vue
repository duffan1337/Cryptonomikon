      ...
      <template v-if="tickers.length !== 0">
        ...
        <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3">
          <div
            ...
            v-bind:key="t.name"
            @click="select(t)"
            :class="selectedTicker === t ? 'border-4' : '' || t.price === '-' ? 'bg-red-100' : 'bg-white'"
...
export default {
  ...
  data() {
    return {
      ...
      allCoins: [],
      elementIsAdded: false, //проверка на добавленный элемент
      hints: [], //подсказки
  ...
  mounted: async function () {
    ...
    window.addEventListener('resize', this.calculatedMaxGrapghElements)
  ...
  beforeUnmount(){
     window.removeEventListener('resize', this.calculatedMaxGrapghElements)
  },
   ...
   updateTicker(tickerName, price){
    ...
    add(ticker) {
      ...
      let isInArray = this.tickers.find(
        (t) => t.name.toLowerCase() === ticker.toLowerCase()
      );
    ...
    onSearchInput() {
      this.elementIsAdded = false;
      this.hints = [];
      var re = new RegExp(this.ticker.toLowerCase());

      for (let i = 0; i < this.allCoins.length; i++) {
        if (re.test(this.allCoins[i].toLowerCase())) {
          this.hints.push(this.allCoins[i]);
        }
  ...
  watch: {
    ...
    selectedTicker() {
      this.graph = [];
        this.$nextTick().then(this.calculatedMaxGrapghElements)
    },
